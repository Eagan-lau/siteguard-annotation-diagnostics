...
if PHASE_ID == 27:
    ...
    PROTOCOL = ROOT / "PROJECT/Phase33_blinded_sample_size_adapted_external_protocol.md"
    LOWER_DATE = "2009-01-01"
    UPPER_DATE = "2016-01-01"
    QUERY_PREFIX = "RCSB33::"
else:
...
def main() -> None:
    ...
    if PHASE_ID == 33:
        ...
        if protocol_lock.get("status") != "FROZEN_BEFORE_ACQUISITION":
            raise RuntimeError("Phase33 protocol lock is not valid")
        if protocol_lock.get("protocol_sha256") != sha256(PROTOCOL):
            raise RuntimeError("Phase33 protocol changed after freeze")
        expected_script = protocol_lock.get("script_sha256", {}).get("scripts/pdb_2009_2015_acquire_rcsb.py")
        if expected_script != sha256(Path(__file__)):
            raise RuntimeError("Phase33 acquisition script changed after freeze")
    ...
    prior_phases = (
        ...
        [27, 28, 30, 31, 32] if PHASE_ID == 33 else []
    )
