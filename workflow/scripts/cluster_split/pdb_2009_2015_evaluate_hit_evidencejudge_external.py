...
BOOTSTRAPS = 10000
SEED = {31: 20260901, 33: 20260929}[PHASE_ID]
...
def main() -> None:
    ...
    if PHASE_ID == 33:
        ...
        if sha256(protocol_lock_path) != lock["protocol_lock_sha256"]:
            raise RuntimeError("Phase33 protocol lock changed after blind inference")
        if any(
            not (ROOT / path).is_file() or sha256(ROOT / path) != expected
            for path, expected in protocol_lock["script_sha256"].items()
        ):
            raise RuntimeError("Phase33 frozen script changed before truth opening")
