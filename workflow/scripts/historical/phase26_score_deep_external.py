#!/usr/bin/env python3
"""Apply the frozen Phase 11 neural pair scorer to external label-blind pairs."""
...
def main() -> None:
    required = CHECKPOINTS / "CHECKPOINT_26A5A_EXTERNAL_PAIR_EVIDENCE_PASS"
    ...
    summary = {"phase": "26A5B", "status": "PASS", "shape": list(output.shape), "model_frozen_phase": 11}
    (ROOT / "reports/phase26_external_deep_score_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    (CHECKPOINTS / "CHECKPOINT_26A5B_EXTERNAL_DEEP_SCORE_PASS").write_text(json.dumps(summary, indent=2) + "\n")
