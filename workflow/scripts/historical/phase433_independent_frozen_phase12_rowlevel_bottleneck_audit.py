...
def independent_source_check(auditor_path: Path) -> dict[str, Any]:
    ...
    forbidden_imports = [name for name in imports if name.startswith(("subprocess", "socket", "requests")) or "phase433_frozen_phase12_rowlevel_bottleneck_closure" in name]
...
def main() -> int:
    ...
    for item in contract.get("implementation_identities", []):
        ...
        if relative.endswith("phase433_independent_frozen_phase12_rowlevel_bottleneck_audit.py"):
            auditor_path = path
